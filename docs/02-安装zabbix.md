...
- 准备好mysql配置文件
    ```shell
    cd /opt/k8s_learning_scripts/laboratory/config_map_files/zabbix-mysql
    kubectl -n laboratory create configmap zabbix-mysql-conf --from-file=my.cnf
    ```
    
- 准备好变量
    ```shell
    cd laboratory/config_map_vars
    kubectl apply -f laboratory/config_map_vars/laboratory-configmap-zabbix.yaml
    ```

- 安装zabbix-mysql
    ```shell
    cd laboratory/zabbix/zabbix-mysql
    kubectl apply -f laboratory-pvc-zabbix-mysql.yaml 
    kubectl apply -f laboratory-deploy-zabbix-mysql.yaml 
    kubectl apply -f laboratory-svc-zabbix-mysql.yaml 
    ```
    
- 安装zabbix-server
    ```shell
    cd laboratory/zabbix/zabbix-server
    kubectl apply -f laboratory-deploy-zabbix-server.yaml
    kubectl apply -f laboratory-svc-zabbix-server.yaml
    ```

- 安装zabbix-web
    ```shell
    cd laboratory/zabbix/zabbix-web
    kubectl apply -f laboratory-deploy-zabbix-web.yaml
    kubectl apply -f laboratory-svc-zabbix-web.yaml
    ```

- 安装zabbix-agent
    ```shell
    cd laboratory/zabbix/zabbix-agent
    kubectl apply -f laboratory-ds-zabbix-agent.yaml
    ```
...
- 代理zabbix服务，nginx主配置文件为laboratory/proxy/nginx/nginx.conf，子配置文件为laboratory/proxy/nginx/conf.d/zabbix.lb.example.com.conf。
...
## 4. 后续优化
- zabbix-agent，部署时，使用daemonset方式部署，还需要挂载本地碰盘目录，用于存放自定义监控项。
